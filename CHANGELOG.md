...
- New sealed `MinHasher<const P: usize, Value: CoreHash = u64>` trait unifies the three sketch types. `MinHash` and `SparseHashes` implement it blanket over any `V: CoreHash`. `SparseValues` implements it only for `Value = u64` because its codec buffer stores raw `u64` inputs. Trait methods are `insert`, `may_contain`, `densify`, `to_dense`, `estimate_jaccard_index(&self, other: &Self)`, and a sealed default `band_hashes::<const BANDS>()` that materialises the dense signature and calls the canonical `MinHash::band_hashes`. Cross-variant Jaccard is done explicitly by calling `to_dense` on both operands and comparing the resulting `MinHash` instances. The sparse-sparse exact fast path fires automatically through the trait for same-variant sparse operands. External crates cannot implement `MinHasher` because the trait is sealed.
...
- `pub mod index` behind the new default `alloc` feature: a compile-time banded LSH index `LshIndex<K, const PERMUTATIONS, const BANDS, S>` over any `MinHasher<PERMUTATIONS, u64>`. Type-level `Store` and `NoStore` markers gate whether the index keeps signatures for a Jaccard refine pass or drops them after banding. Query is one binary search per band followed by a collision-count aggregation, backed by a caller-owned `QueryState` scratch for zero-allocation queries. Ranked `candidates` and `top_k` (Store only) build on the same primitive. Turning `alloc` off yields the previous no-`Vec` surface unchanged.
